...
export default async function ProjectDetailPage({
  ...
  const clients = project.members.map((member) => member.profile).filter((profile) => profile.role === "client");
          ...
          <aside className="grid gap-6">
            <article className="card-shadow rounded-[2rem] border border-[color:var(--line)] bg-white p-6">
              <div className="border-b border-[color:var(--line)] pb-4">
                ...
                <h2 className="mt-2 text-3xl font-semibold">Reserva de trabajo</h2>
              </div>
                  ...
                  >
                    ...
                    {availablePurchases.map((purchase) => (
                      <option key={purchase.id} value={purchase.id}>
                        {purchase.sessionPackage?.name ?? "Paquete"} · {purchase.sessionsRemaining} restantes
                      </option>
                    ))}
                  </select>
                </label>
                  ...
                  project.purchases.map((purchase) => (
                    ...
                    >
                      ...
                      <p className="mt-2 text-sm text-white/75">
                        Estado: {purchase.status} · Restantes: {purchase.sessionsRemaining} / {purchase.sessionsTotal}
                      </p>
                    </div>
                  ))
                )}
